...
const REGISTRY_KEY = "ai_usage_kimi_account_registry_v1";
...
function secretKey(profileId: string, field: string): string {
  return `ai_usage_kimi_profile_${profileId}_${field}`;
}
...
export function createAccount(name = ""): KimiAccountProfile {
  ...
  const profile: KimiAccountProfile = {
    id: makeId(),
...
export function getStableDeviceId(): string {
  const existing = getSecretRaw(DEVICE_ID_KEY);
  if (existing) return existing;
  const bytes = Crypto.generateSymmetricKey(128);
  const id = Array.from(bytes)
    .map((byte) => byte.toString(16).padStart(2, "0"))
    .join("");
  setSecretRaw(DEVICE_ID_KEY, id);
  return id;
}
