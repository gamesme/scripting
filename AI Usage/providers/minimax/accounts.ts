import type { AccountRegistry, MinimaxAccountProfile, MinimaxRegion } from "./types";

const REGISTRY_KEY = "ai_usage_minimax_account_registry_v1";
...
function secretKey(profileId: string, field: string): string {
  return `ai_usage_minimax_profile_${profileId}_${field}`;
}
...
export function createAccount(name = ""): MinimaxAccountProfile {
  ...
  const profile: MinimaxAccountProfile = {
    id: makeId(),
...
export function deleteAccount(profileId: string): void {
  ...
  for (const field of ["access_token", "region", "account_id"])
...
export function getProfileRegion(profileId?: string | null): MinimaxRegion | null {
  const profile = resolveProfile(profileId);
  const raw = profile ? getSecretRaw(secretKey(profile.id, "region")) : null;
...
export function saveProfileCredentials(
  ...
  if (value.region) setSecretRaw(secretKey(profile.id, "region"), value.region);
  if (value.accountId)
